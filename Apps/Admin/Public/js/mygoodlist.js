      ...
      if (len > 0) {
        for (var i = 0; i < len; i++) {
          var str = ' <li><div class="thumbnail"><img style="height:185px;" class="goodpicsmall" src="' + publicUrl + data[i].goodimg1 + '"><div class="caption">' +
            '<h4 class="goodname">' + data[i].goodname;
          if (data[i].status == 0) {
            str = str + '<small>暂时下架</small>'
          }
          str = str + '</h4><p class="gooddetail">' + data[i].gooddetail + '</p>';

          str = str + ' <p style="text-align:center"> <a class="btn btn-danger delbutton" role="button"  data-id="' + data[i].good_id + '">删 除</a></p></div></div></li>';
          arr.push(str);
        }
      } else {
